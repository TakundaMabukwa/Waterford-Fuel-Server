...
async function importDaily41() {
  console.log('📊 IMPORTING Daily (41).xlsx (2026-01-22 ONLY)\n');
  
  try {
    // Delete existing sessions for 2026-01-22
    console.log('🗑️  Deleting existing sessions from 2026-01-22...');
    const { error: deleteError } = await supabase
      .from('energy_rite_operating_sessions')
      .delete()
      .eq('session_date', '2026-01-22');
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Parse session data - ONLY for 2026-01-22
      if (col1 && col2 === '2026-01-22') {
          ...
          if (error) {
            ...
            if (session.total_fill > 0) {
              const fillSession = {
                ...
                notes: `Fuel fill: ${session.total_fill}L. Imported from Daily (41).xlsx`
              };
...
function parseSessionRow(row, site, runningTimes) {
  ...
  if (!dateStr || dateStr !== '2026-01-22' || !operatingHours) return null;
  
  const hours = parseHours(operatingHours);
  if (!hours || hours <= 0) return null;
  
  const sessionDate = new Date('2026-01-22');
  ...
  // Skip sessions with 0 closing fuel (incomplete/cross-day sessions)
  if (closingFuel === 0 && totalUsage > 0) {
    console.log(`⚠️  Skipping ${site} - incomplete session (0 closing fuel)`);
    return null;
  }
  ...
  return {
    ...
    notes: `Imported from Daily (41).xlsx`
  };
}
